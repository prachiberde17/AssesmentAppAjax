package com.assessment.data;

public class OnetAnswer {

	private int one1;
	private int two1;
	private int three1;
	private int four1;
	private int five1;
	private int six1;
	private int seven1;
	private int eight1;
	private int nine1;
	private int ten1;
	private int eleven1;
	private int twelve1;

	private boolean first;
	private int next;
	private int page;
	private boolean last;

	public boolean isLast() {
		return last;
	}

	public void setLast(boolean last) {
		this.last = last;
	}

	public boolean isFirst() {
		return first;
	}

	public void setFirst(boolean first) {
		this.first = first;
	}

	public int getNext() {
		return next;
	}

	public void setNext(int next) {
		this.next = next;
	}

	public int getPage() {
		return page;
	}

	public void setPage(int page) {
		this.page = page;
	}

	public int getTwo1() {
		return two1;
	}

	public void setTwo1(int two1) {
		this.two1 = two1;
	}

	public int getThree1() {
		return three1;
	}

	public void setThree1(int three1) {
		this.three1 = three1;
	}

	public int getFour1() {
		return four1;
	}

	public void setFour1(int four1) {
		this.four1 = four1;
	}

	public int getFive1() {
		return five1;
	}

	public void setFive1(int five1) {
		this.five1 = five1;
	}

	public int getSix1() {
		return six1;
	}

	public void setSix1(int six1) {
		this.six1 = six1;
	}

	public int getSeven1() {
		return seven1;
	}

	public void setSeven1(int seven1) {
		this.seven1 = seven1;
	}

	public int getEight1() {
		return eight1;
	}

	public void setEight1(int eight1) {
		this.eight1 = eight1;
	}

	public int getNine1() {
		return nine1;
	}

	public void setNine1(int nine1) {
		this.nine1 = nine1;
	}

	public int getTen1() {
		return ten1;
	}

	public void setTen1(int ten1) {
		this.ten1 = ten1;
	}

	public int getEleven1() {
		return eleven1;
	}

	public void setEleven1(int eleven1) {
		this.eleven1 = eleven1;
	}

	public int getTwelve1() {
		return twelve1;
	}

	public void setTwelve1(int twelve1) {
		this.twelve1 = twelve1;
	}

	public int getOne1() {
		return one1;
	}

	public void setOne1(int one1) {
		this.one1 = one1;
	}

}
